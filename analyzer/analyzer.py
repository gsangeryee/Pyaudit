...
from .signature_extractor import generate_signature_md
...
class Analyzer(object):
    ...
    def analyze(self, serverity, rules, paths):
        ...
        for rule in rules:
            ...
            if rule['identifier'] == "[G-017]":
                print("rule pattern:", rule['pattern'])
            ...
            # if rule['identifier'] == "[G-033]": directly input rule_indentifier,
            # rule_title, rule_description, rule_recommendation and empty findings into findings_per_rule_dict
                ...
                with open(path, 'r') as f:
                    ...
                    if pattern_type == "Normal":
                        ...
                        if len(findings) > 0:
                            ...
                            for finding in findings:
                                ...
                                # if finding is a multi-line, only use the first line
                                lines = finding.splitlines()
                                first_line = lines[0]
                                
                                finding_dict_list = self.add_finding_item(
                                    first_line, file_name, path)
                    ...
                    if pattern_type == "Group":
                        ...
                        if len(findings) > 0:
                            ...
                            for finding in findings:
                                finding_dict_list = []
                                finding_dict = self.add_finding_item(
                                    finding, file_name, path)
                                # findings_list.append(finding_dict)
                                findings_list.extend(finding_dict_list)
                    ...
                    if pattern_type == "Multi":
                        ...
                        if len(findings) > 0:
                            ...
                            # remove duplicate findings
                            findings = set(findings)
                            # print("Multi findings Remove space and duplicate:",findings)
                            for finding in findings:
                                finding_dict_list = []  # dictionary for finding, initialize must in the loop
                                # One Line
                                ...
                                if len(finding.splitlines()) > 1:
                                    ...
                                    if second_line.strip() == "":
                                        second_line = finding.splitlines(
                                            False)[2]  # Get the third line
                                        back_number = 2
                                        # skip the third line,using the fourth line  if it is a comment line
                                        if second_line.strip().find('//') == 0:
                                            second_line = finding.splitlines(False)[
                                                3]
                                            back_number = 3
                                    else:
                                    ...
                                    for m_line_number, m_line in enumerate(contract_file):
                                        ...
                                        if m_head.strip() == second_line:
                                            second_line_number_list.append(
                                                m_line_number+1)
                                    contract_file.close()
                                    ...
                                    for second_line_number in second_line_number_list:
                                        ...
                                        finding_dict['line_number'] = str(
                                            first_line_number)+"-"+str(first_line_number+len(finding.splitlines(False))-1)
                    ...
                    if pattern_type == "Sub":
                        ...
                        #print("rule['sub_pattern']", rule['sub_pattern'])
                        ...
                        sub_finding_list = list(matches)
                        #print("matches:",sub_finding_list)
                        #print("len(matches):",len(sub_finding_list))
                        if len(sub_finding_list) > 0:
                            # remove empty space front and back in findings
                            #sub_finding_list = [sub_finding.strip()
                            #                   for sub_finding in sub_finding_list]
                            # remove duplicate findings
                            #sub_finding_list = set(sub_finding_list)              
                            
                            for match in sub_finding_list:
                                sub_finding = match.group(0).strip()
                                #print("matched_string:",sub_finding_list)
                                try:
                                    ...
                                    #print("group_result:",sub_key)
                                except IndexError:
                                    sub_key = ""
                                    print("IndexError:",sub_key)
                                
                                sub_pattern = sub_pattern_string.replace(
                                    'SUB_PATTERN', sub_key)
                                sub_rule_regex = re.compile(sub_pattern)

                                sub_findings = sub_rule_regex.findall(code)
                                # if rule is [G-018] print("sub_pattern:",sub_pattern)
                                """
                                if rule['identifier'] == "[G-017]":
                                    print("sub_pattern:", sub_pattern)
                                    print("sub_findings:", sub_findings)
                                    print("len(sub_findings):", len(sub_findings))
                                ...
                                if sub_pattern_type == "One":
                                    if len(sub_findings) == 1:
                                        ...
                                        # if sub_finding is a multi-line, only use the first line
                                        lines = sub_finding.splitlines()
                                        first_line = lines[0]

                                        # for "One" finding is the farther finding = sub_finding
                                        sub_finding_dict_list = self.add_finding_item(
                                            first_line, file_name, path)
                                        """
                                        if rule['identifier'] == "[G-018]":
                                            print("sub_finding_dict_list:", sub_finding_dict_list)
                                        """
                                ...
                                if sub_pattern_type == "Two":
                                    if len(sub_findings) == 2:
                                        ...
                                        # for "Two" finding is the farther finding = sub_finding
                                        sub_finding_dict_list = self.add_finding_item(
                                            first_line, file_name, path)
                                        # findings_list.append(sub_finding_dict)
                                        findings_list.extend(
                                ...
                                if sub_pattern_type == "Sub_Normal":
                                    if len(sub_findings) > 0:
                                        for sub_find in sub_findings:
                                            ...
                                            sub_finding_dict_list = self.add_finding_item(
                                                sub_find, file_name, path)
                                            # findings_list.append(sub_finding_dict)
                                            findings_list.extend(
                                                sub_finding_dict_list)
                                            

                    # Using the generate_signature_md to output METHOD ID of function name for [G-036]
                    markdown_file_name = generate_signature_md(path)
                    #print(f"Markdown file '{markdown_file_name}' has been generated successfully.")
